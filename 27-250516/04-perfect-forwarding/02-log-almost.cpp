...
int main() {
    ...
    int x = 20;
    log(bar, x);  // No more std::ref
    std::cout << "x = " << x << "\n";

    std::string s = "hello world from long string";
    const std::string cs = "hello world from long const string";
    Storer baz;
    log(baz, s);  // 1 copy assignment
    log(baz, std::move(s));  // 1 move assignment
    log(baz, cs);  // 1 copy assignment
    log(baz, std::move(cs));  // 1 copy (because it's rvalue reference to const)
