...
* не было: синтаксис объявления пространств имён, обращения к элементу внутри пространства имён
...
* Перегрузка функций: простые примеры
  * Не было: что входит в сигнатуру, что не входит (например, значение по умолчанию).
  * Не было: типы-тэги для пояснений к перегрузкам (type tags)
  * Не было: проблемы с разделением `nullptr`, `NULL`, `0`.
  * Не было: `noexcept`.
...
* Отличия указателей от ссылок: изменяемость, обнуляемость, хранение в контейнерах, продление жизни
...
* Отличия `unique_ptr` / `shared_ptr` от обычного указателя, в каких случаях что использовать
* Создание `unique_ptr` / `shared_ptr`: какие есть конструкторы, что делает `make_unique` / `make_shared`
  * Не было: проблема с `make_unique` / `make_shared` и приватными конструкторами
    * Решения: неработающее с друзьями, работающее с фабричной статической функцией
* Невозможность копирования
* Синтаксис перемещения: в параметры функции, из функции, в другие/из других переменных (включая поля), в контейнеры/из контейнеров
...
* Не было: copy-swap-idiom для упрощения до правила четырёх
...
* Статические поля:
  ...
  * Определения снаружи класса
  * Не было: доступ к приватным членам в инициализаторе статического поля при определении снаружи класса.
  * Не было: порядок инициализации, удаления
...
* External linkage/internal linkage.
  ...
  * Не было: `static`, отличия от unnamed space
...
* Не было: пример неполиморфного наследование при реализации двусвязного списка с фиктивным элементом
...
* Создание и использование разделяемой библиотеки через Boost.DLL для системы плагинов на основе полиморфных классов (`08-231107/10-shared-library`)
  ...
  * Не было: загрузка и выгрузка разделяемой библиотеки; что при этом происходит с её глобальными переменными
