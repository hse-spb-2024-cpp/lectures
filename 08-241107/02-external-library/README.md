...
Binaries are different for different compilers and settings, require *ABI compatibility* (application binary interface)
...
* Dynamic library. `*.dll` on Windows, `lib*.so` on Linux (shared object), `*.dylib` on macOS.
